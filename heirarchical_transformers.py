...
class NodeAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        present = (None,)
        attn_outputs = self._attn(query, key, value, edge_matrix, attention_mask, output_attentions)
        a = attn_outputs[0]
...
class GlobalTransfoXLModeller(nn.Module):
    ...
    def forward(
        ...
        if self.same_length:
            ...
            dec_attn_mask = torch.triu(time_embedding.new_ones((qlen, klen), dtype = torch.uint8), diagonal = 1 + mlen)[:, :, None]

        hids = []
        attentions = [] if output_attentions else None
        pos_seq = torch.arange(klen-1, -1, -1.0, device = time_embedding.device, dtype = time_embedding.dtype)
        if self.clamp_len > 0:
